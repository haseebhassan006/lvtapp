    ...
    <Disclosure as="nav" class="bg-gray-800" v-slot="{ open }">
                ...
                <transition enter-active-class="transition ease-out duration-100" enter-from-class="transform opacity-0 scale-95" enter-to-class="transform opacity-100 scale-100" leave-active-class="transition ease-in duration-75" leave-from-class="transform opacity-100 scale-100" leave-to-class="transform opacity-0 scale-95">
                  <MenuItems class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <MenuItem   v-slot="{ active }">
                      <a @click='signout' :class="['block px-4 py-2 text-sm text-gray-700']">Sign Out</a>
                    </MenuItem>
                  </MenuItems>
                </transition>
              </Menu>
      ...
      <DisclosurePanel class="md:hidden">
        <div class="px-2 pt-2 pb-3 space-y-1 sm:px-3">
          ...
          'block px-3 py-2 rounded-md text-base font-medium']" >{{ item.name }}</DisclosureButton>
        </div>
        <div class="pt-4 pb-3 border-t border-gray-700">
         ...
         <div class="mt-3 px-2 space-y-1">
            <DisclosureButton   @click="logout" class="block px-3 py-2 rounded-md cursor-pointer text-base font-medium text-gray-400 hover:text-white hover:bg-gray-700">Sign Out</DisclosureButton>
          </div>
        </div>
      </DisclosurePanel>
    </Disclosure>
